...
export default function ProdutosServicosManager({ tenantId, itens, readOnly }: { tenantId: string; itens: ProdutoServico[]; readOnly: boolean }) {
  const [formOpen, setFormOpen] = useState<'none' | 'new' | ProdutoServico>('none')
  const [erro, setErro] = useState<string | null>(null)
  const [state, formAction, pending] = useActionState<CadastroFormState, FormData>(upsertProdutoServico, {})
  ...
  return (
    <div>
      ...
      {!readOnly && formOpen !== 'none' && (
        <form action={formAction} className="bg-[var(--off)] rounded-xl p-4 flex flex-col gap-3 mb-5 max-w-lg">
          ...
          <div className="grid grid-cols-2 gap-3">
            <select name="tipo" defaultValue={editing?.tipo ?? 'produto'} className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--page-bg)] text-sm">
          ...
          <input name="nome" required defaultValue={editing?.nome} placeholder="Nome" className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--page-bg)] text-sm" />
          <div className="grid grid-cols-3 gap-3">
            <input name="sku" defaultValue={editing?.sku ?? ''} placeholder="SKU" className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--page-bg)] text-sm" />
            <input name="preco" defaultValue={editing?.preco ?? ''} placeholder="Preço" inputMode="decimal" className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--page-bg)] text-sm" />
            <input name="unidade" defaultValue={editing?.unidade ?? 'un'} placeholder="Unidade" className="px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--page-bg)] text-sm" />
          </div>
      ...
      {itens.length === 0 ? (
        <p className="text-sm text-[var(--muted)]">Nenhum produto ou serviço cadastrado ainda.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] text-left">
                ...
                <th className="px-4 py-2.5 font-medium text-[var(--muted)] hidden sm:table-cell">Preço</th>
              ...
              {itens.map(p => (
                <tr key={p.id}>
                  <td className="px-4 py-2.5 text-[var(--ink)] font-medium">
                    {p.nome}
                    <span className="ml-2 text-[10px] font-semibold text-[var(--muted)] uppercase">{p.tipo}</span>
                  </td>
                  <td className="px-4 py-2.5 text-[var(--muted)] hidden sm:table-cell">
                    {p.preco != null ? `R$ ${Number(p.preco).toFixed(2)} / ${p.unidade}` : '—'}
                  </td>
