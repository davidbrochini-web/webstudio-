...
import { updateCoresLocaldesk } from '@/app/app/(hub)/localdesk/cores/actions'

// Presets — inclui a paleta padrão do LocalDesk (azul/verde, herança
// técnica de TI) e algumas variações rápidas.
const PRESETS = [
  { nome: 'Azul/Verde (padrão)', primaria: '#1E4FD8', secundaria: '#10151F' },
  { nome: 'Azul escuro', primaria: '#1E40AF', secundaria: '#0F172A' },
  { nome: 'Verde técnico', primaria: '#12B886', secundaria: '#0B2B22' },
  { nome: 'Roxo tech', primaria: '#6D28D9', secundaria: '#1E1B2E' },
]

export default function LocaldeskCoresEditor({ siteId, corPrimariaInicial, corSecundariaInicial, readOnly }: {
  siteId: string
  ...
  function salvar() {
    ...
    startTransition(async () => {
      try { await updateCoresLocaldesk(siteId, primaria, secundaria); setSalvo(true) }
