...
export default function PortfolioLayout({ config }: { config: NicheConfig }) {
  const { businessName, tagline, heroTitle, ctaLabel, accent, solidBg, services, posts, testimonials, igHandle, photoIds, faq, planos, blogPosts, siteId } = config
  ...
  return (
    <>
      ...
      <LeadForm siteId={siteId} accent={accent} solidBg={solidBg} heading="Vamos criar algo juntos?" subtext="Conte um pouco da sua ideia e retornamos com uma proposta." />

      {/* CTA */}
      <section className={`bg-gradient-to-br ${accent} px-6 py-16 sm:py-20 text-center`}>
        <div className="max-w-xl mx-auto">
          <h2 className="font-display font-extrabold text-2xl sm:text-3xl text-white mb-4">{config.ctaHeading || "Vamos criar algo juntos?"}</h2>
          <a href={WA_LINK} target="_blank" rel="noopener noreferrer"
