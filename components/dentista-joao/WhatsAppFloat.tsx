 ...
 * - Animação de pulso (anel expansivo) pra chamar atenção no mobile
 * - Label tooltip que aparece no hover/foco (desktop)
 * - Esconde o pulso após 1ª interação pra não poluir depois
 */
export default function WhatsAppFloat({ whatsapp }: { whatsapp: string | null }) {
  const [pulsed, setPulsed] = useState(true)
  if (!whatsapp) return null
  const link = `https://wa.me/${whatsapp.replace(/\D/g, '')}?text=Olá%2C%20gostaria%20de%20agendar%20uma%20consulta`
