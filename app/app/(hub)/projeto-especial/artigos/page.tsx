...
const FIELDS: FieldConfig[] = [
  ...
  { name: 'conteudo', label: 'Conteúdo completo do artigo', type: 'textarea', required: true },
  { name: 'capa_url', label: 'URL da imagem de capa' },
  { name: 'alt_text', label: 'Texto alternativo da capa' },
  { name: 'meta_titulo', label: 'Meta título (SEO)' },
  { name: 'meta_descricao', label: 'Meta descrição (SEO)', type: 'textarea' },
...
export default async function ArtigosAdminPage() {
  ...
  const { data: itens } = await supabase
    ...
    .select('id, titulo, slug, resumo, conteudo, capa_url, alt_text, meta_titulo, meta_descricao, ordem, publicado')
  ...
  return (
    <div>
      ...
      <ConteudoManager
        ...
        addLabel="Adicionar artigo"
        emptyLabel="Nenhum artigo cadastrado ainda."
        readOnly={!podeEditar}
      />
    </div>
  )
