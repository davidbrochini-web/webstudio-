...
export default async function LeadsPage() {
  ...
  const { data: leads } = await supabase
    .from('site_leads')
    .select('id, nome, contato, mensagem, created_at')
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="text-[var(--muted)] text-sm mb-8">Contatos e pedidos de visita vindos do seu site.</p>

      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Hoje', valor: hoje, cor: hoje > 0 ? '#1B3A6B' : undefined },
      ...
      {!leads?.length ? (
        ...
        <div className="flex flex-col gap-3">
          {leads.map(l => {
            ...
            return (
              <div key={l.id}
                className={`bg-[var(--card-bg)] rounded-2xl border p-5 ${ehHoje ? 'border-[#1B3A6B]/40 shadow-sm' : 'border-[var(--border)]'}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-bold text-[var(--ink)] text-base">{l.nome}</p>
                      {ehHoje && <span className="text-[10px] font-bold bg-[#1B3A6B] text-white px-2 py-0.5 rounded-full">Novo</span>}
                    </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-xs text-[var(--muted)]">{data.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}</p>
                    <p className="text-xs text-[var(--muted)]">{data.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                </div>
              </div>
            )
